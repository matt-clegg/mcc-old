...
function enableinvites() {
	...
	// code mirror is broken and won't display the content until editors are refreshed!  See
	// See comment on code mirror plugin code "Some browsers do something weird with the fieldset which doesn't work well with CodeMirror. Fix it."
	/*
	...
	// do try firing it
	try {
		jQuery(document.body).trigger('shown');
	}
	catch (e)
	{
...
function checkUserExists(e, elem, checkurl) {
...
function rsvpaddInvitee(invitee) {
	...
	jQuery("#rsvpupdateinvites").css("display","inline-block");
	jQuery("#rsvpemailinvites").css("display","inline-block");
	jQuery("#rsvpreemailinvites").css("display","inline-block");
...
function inviteFriends(checkurl) {
	addInvitees();
...
function inviteJSGroup(checkurl, groupid) {
	if (groupid == "NONE")
		return false;
	addInvitees();
...
function inviteJoomlaGroup(checkurl, groupid) {
	if (groupid == "NONE")
		return false;
	addInvitees();
...
function inviteCBGroup(checkurl, groupid) {
	if (groupid == "NONE")
		return false;
	addInvitees();
...
function inviteList(checkurl, listid) {
	if (listid == "NONE")
		return false;
	addInvitees();
...
function showSubmitButton() {
	jQuery("#jevattendsubmit").css("display","block");
...
var jevrsvpRequiredFields = {
	...
	verify:function (form){
		...
		var lastGuest = jQuery('#lastguest').val();
		for (var lg = 0; lg < lastGuest; lg++)
		{
			var tabLi = jQuery('#' + regTabs.elid + ' ul.nav-tabs li, #' + regTabs.elid + ' ul.uk-tab li');
			if (tabLi[lg]) {
				tabLi[lg].classList.remove('required_tab');
			}
		}
		...
		jQuery(jevrsvpRequiredFields.fields).each(function (i, item) {
			...
			if (item.requiredCheckScript && window[item.requiredCheckScript]) {
				...
				jQuery(matches).each (function (index, match){
					...
					value = match.value;
					// chosen replacements!
					if (jQuery('#'+item.id+'_chzn').length)
					{
					...
					if (JevrConditionalFields.isVisible(conditionalfieldname, index) && (value == item['default'] || value == "" )){
						...
						jQuery(match).parents('.paramlist_value').parent().addClass("required_field");

						if(item.reqmsg!=""){
							messages.push(item.reqmsg);
						}

						jevrsvpRequiredFields.highlightTab(match);
						...
						jQuery(match).removeClass("required_field");
						jQuery(match).parents('.paramlist_value').parent().removeClass("required_field");
					}
				});
...
/*
		var attendSubmit = document.getElementById('jevattendsubmit');
		var clicktoattend = document.getElementById('clicktoattend');
		var nextattendstep = document.getElementById('nextattendstep');
		if (messages.length == 0 && attendSubmit.value == nextattendstep.value)
		{
			attendSubmit.value = clicktoattend.value;
			document.getElementById('addkillguests').style.opacity = 0.5;
			document.getElementById('rsvppro_admintable').style.opacity = 0.5;
			document.getElementById('addkillguests').setAttribute('hidden');
			document.getElementById('rsvppro_admintable').style.display = 'none';
			return false;
		}
		else if (messages.length !== 0 && attendSubmit.value == clicktoattend.value)
		{
			attendSubmit.value = nextattendstep.value;
			document.getElementById('addkillguests').style.opacity = 1;
			document.getElementById('rsvppro_admintable').style.opacity = 1;
			document.getElementById('addkillguests').removeAttribute('hidden');
			document.getElementById('rsvppro_admintable').style.display = 'table';
			return false;
		}
 */
	...
	verifyRadio : function(item, form, messages){
		...
		jQuery(matches).each (function ( index, match){
			...
			if (JevrConditionalFields.isVisible(conditionalfieldname, index) && (value == item['default'] || value == "")){
				...
				jevrsvpRequiredFields.highlightTab(match.closest('.rsvpparam'));
	...
	verifyCheckbox : function(item, form, messages){            
		...
		if (failures.length>0){
			...
			if (item.requiredtype==2 && matches.length>0) {
				...
				//highlight the bad element values
				jQuery(failure).parents('.paramlist_value').parent().addClass("required_field");
				...
				jevrsvpRequiredFields.highlightTab(failure.closest('.rsvpparam'));
			});
		}
		return messages;
	},
	emailcheck : function () {
		...
		var str = jQuery("#jevattend_email").val().trim();
	...
	highlightTab: function(match)
	{
		...
		for (var lg = 0; lg <= lastGuest; lg++)
		{
			if (match.classList.contains('rsvpparam'  + lg))
			{
				var tabLi = jQuery('#' + regTabs.elid + ' ul.nav-tabs li , #' + regTabs.elid + ' ul.uk-tab li ');
				if (tabLi[lg]) {
					tabLi[lg].classList.add('required_tab');
				}
			}
		}
...
function addGuest(limit) {
	...
	if (myElem.length) {
		jQuery('html, body').animate({
			scrollTop: myElem.offset().top-300
		}, 400);
...
function removeGuest(limit) {
	...
	if (limit > 0 && jQuery('#guestcount').val() < limit) {
		if (jQuery('#addguest').length)
			jQuery('#addguest').css('display', "unset");
	}
...
function setTemplate(id, title)
{
	...
	try {
		// GSL form replacement
		gslselect("#custom_rsvp_template");
	}
	catch (e) { }
...
function changeTemplateSelection() {
	...
	if (!(selectedNode.val().indexOf(".xml") > 0 || selectedNode.val() == 0)) {
			...
			if ($cfels && $cfels.length >= 1) {
				if ($cfels.length == 1) {
					...
					if ($cfel.length) {
						...
						$cfel.find("select").each(function (sindex, sadel) {
							$sadel = jQuery(sadel);
							$sadel.off('change', JevrConditionalFields.setup);
							$sadel.on('change', JevrConditionalFields.setup);

							if ($sadel.val() == cf.cfvfv){
								jQuery('.param' + cf.name).removeClass("conditionalhidden");
							}
							else {
								jQuery('.param' + cf.name).addClass("conditionalhidden");
					...
					$cfels.each(function (cfelindex, cfel) {
						...
						if (jQuery(cfel).hasClass("paramtmpl") || jQuery(cfel).hasClass("hideparam")) return;
						...
						jQuery(cfel).find("select").each(function (sindex, sadel) {
							$sadel = jQuery(sadel);
							$sadel.off('change', JevrConditionalFields.setup);
							$sadel.on('change', JevrConditionalFields.setup);
				...
				if ($cfels && $cfels.length >= 1) {
					if (field.peruser == 0) {
						...
						$cfels.each(function (cfelindex, cfel) {
							...
							// some might be spans etc so find the embedded select lists
							$cfel.find("select").each(function (sindex, sadel) {
								// this is the one we are looking for with the value that matches the condition value and its checked
								$sadel = jQuery(sadel);
...
function toggleSessionAccessMessage() {
	if (document.querySelector("[name=custom_rsvp_sessionaccess]").value > 0) {
